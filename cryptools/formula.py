...
def get_prime(n):
    ...
    for i in range(1,mpz(n**0.5/3)+1):
      if sieve[i]:
        k=3*i+1|1
        sieve[      k*k//3      ::2*k] = [False] * mpz((n//6-k*k//6-1)//k+1)
        sieve[k*(k-2*(i&1)+4)//3::2*k] = [False] * mpz((n//6-k*(k-2*(i&1)+4)//6-1)/k+1)
    return [2,3] + [3*i+1|1 for i in range(1,mpz(n/3-correction)) if sieve[i]]
