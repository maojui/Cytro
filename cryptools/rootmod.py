...
def cube_root(c,q):
    """
    If c = m^3 mod q :
    cube_root(c,q) -> return all cube root of c, one of them will be m.

    Only works if the sageworks() function returned True
    """
    import cryptools
    filename = os.path.join(os.path.dirname(cryptools.__file__),'cube_root.sage.py')
    if sageworks():
        sageresult = subprocess.check_output(['sage', filename,str(c), str(q)])
        return list(map(int,sageresult[1:sageresult.find(b'\n')-1].replace(b'L',b'').split(b', ')))


def rth_root(c,q,r):
    """
    If c = m^r mod q :
    rth_root(c,q,r) -> return all rth root of c, one of them will be m.

    Only works if the sageworks() function returned True
    """
    import cryptools
    filename = os.path.join(os.path.dirname(cryptools.__file__),'rth_root.sage.py')
    if sageworks():
        sageresult = subprocess.check_output(['sage', filename,str(c), str(q),str(r)])
        return list(map(int,sageresult[1:sageresult.find(b'\n')-1].replace(b'L',b'').split(b', ')))