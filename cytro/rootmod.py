...
def cube_root(c,q):
    ...
    filepath = os.path.join(os.path.dirname(sageworks.__file__),'sage/cube_root.sage.py')
    with Sage() as sage :
        sage.load(filepath)
        roots = list(map(int,sage.run(f'result = cube_root({c},{q})\n')))
    return roots
...
def rth_root(c,q,r):
    ...
    Only works if the sageworks() function returned True
    """
    filepath = os.path.join(os.path.dirname(sageworks.__file__),'sage/rth_root.sage.py')
    with Sage() as sage :
        sage.load(filepath)
        roots = list(map(sage.run(f'result = rth_root({c},{q},{r})\n')))
    return roots
